...
public class SpidermanKit extends Kit {
    ...
    public SpidermanKit() {
        super("Spiderman", "&cSpiderman", "30s");
    ...
    public void execute(PlayerInteractEvent event) {
        ...
        if (player.getItemInHand() == null || player.getItemInHand().getType() != Material.SNOW_BALL) return;
        Cooldown cooldown = KitPvP.getCooldown(getName());
        if (cooldown.isOnCooldown(player) || profile.getPlayerState() == PlayerState.SPAWN) {
    ...
    public void onLoad(Player player) {
        ...
        inventory.setItem(1, new ItemMaker(Material.SNOW_BALL).setDisplayname(profile.getTheme().getPrimaryColor() + "Sticky Ability").setAmount(32).setUnbreakable(true).create());
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        if (event.getEntity() instanceof Player && event.getDamager() instanceof Snowball) {
            if (((Snowball) event.getDamager()).getShooter() instanceof Player) {
                ...
                if (!(ProfileManager.getProfile(damager).getCurrentKit() instanceof SpidermanKit)) {
                    return;
                }
                if (damager == damaged) {
                    return;
                ...
                FallingBlock block = damaged.getWorld().spawnFallingBlock(location, Material.WEB, (byte) 0);
                Vector vector = location.getDirection().multiply(2);
                block.setVelocity(vector);
                TaskUtil.runTaskLater(() -> location.getBlock().setType(Material.AIR), 5 * 20L);
            }
        }
    }
}