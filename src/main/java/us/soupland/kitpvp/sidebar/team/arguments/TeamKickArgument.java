...
public class TeamKickArgument extends KitPvPArgument {
    ...
    public void onExecute(CommandSender sender, String label, String[] args) {
        ...
        if (!team.getOfficers().contains(((Player) sender).getUniqueId())) {
            sender.sendMessage(ColorText.translate("&cYou must be an officer to kick players."));
            return;
        }
        if (args.length < 2) {
            ...
            if (sender == target) {
                sender.sendMessage(ColorText.translate("&cYou can't kick yourself."));
                return;
            }
            ...
            if (team.getLeader().equals(target.getUniqueId())) {
                sender.sendMessage(ColorText.translate("&cYou can't kick the leader."));
                return;
            }
            if (team.getOfficers().contains(target.getUniqueId()) && team.getLeader() != ((Player) sender).getUniqueId()) {
                sender.sendMessage(ColorText.translate("&cYou can't kick another officer."));
                return;
            }
            ...
            team.sendMessage(ColorText.translate("&ePlayer " + target.getName() + " &ehas been kicked by &c" + sender.getName() + " &efrom the team."));
