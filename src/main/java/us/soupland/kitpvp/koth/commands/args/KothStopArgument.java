...
public class KothStopArgument extends KitPvPArgument {
    ...
    public void onExecute(CommandSender sender, String label, String[] args) {
        ...
        if (!koth.isActive()) {
            sender.sendMessage(ColorText.translateAmpersand("&c" + kothName + " KoTH is not started. &7(/koth start " + kothName + ')'));
            return;
        }

        koth.setActive(false);
        Bukkit.broadcastMessage(ColorText.translateAmpersand("&4&l[KoTH] &9" + kothName + " &chas been stopped."));
    }
}