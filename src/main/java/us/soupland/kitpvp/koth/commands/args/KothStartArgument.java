...
public class KothStartArgument extends KitPvPArgument {
    ...
    public void onExecute(CommandSender sender, String label, String[] args) {
        ...
        if (koth.isActive()) {
            sender.sendMessage(ColorText.translate("&c" + kothName + " KoTH is already started. &7(/koth stop " + kothName + ')'));
            return;
        }
        ...
        if (seconds <= 0) {
            sender.sendMessage(ColorText.translate("&cInvalid seconds format."));
            return;
        }
        ...
        koth.setActive(true);
        Bukkit.broadcastMessage(ColorText.translate("&4&l[KoTH] &9" + kothName + " &ehas started. &6(" + DurationFormatUtils.formatDuration((seconds * 1000L), "mm:ss") + ')'));
